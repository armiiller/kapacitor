...
func (s *Service) TestOptions() interface{} {
	return &testOptions{
		...
		Description: "test pagertree message",
...
func (s *Service) Alert(serviceKey, incidentKey, desc string, level alert.Level, details string) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		r := &response{Message: fmt.Sprintf("failed to understand PagerTree response. code: %d content: %s", resp.StatusCode, string(body))}
...
func (s *Service) preparePost(serviceKey, incidentKey, desc string, level alert.Level, details string) (string, io.Reader, error) {
	...
	case alert.Warning, alert.Critical:
		eventType = "trigger"
	case alert.Info:
		return "", nil, fmt.Errorf("AlertLevel 'info' is currently ignored by the PagerTree service")
	default:
		eventType = "resolve"
	}
...
func (h *handler) Handle(event alert.Event) {
	if err := h.s.Alert(
		...
		h.diag.Error("failed to send event to PagerTree", err)
	}
}
